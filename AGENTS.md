...
| `cuentas/` | el modelo de usuario propio (`Usuario`, correo como identificador), el formulario de alta, el adaptador de `allauth` y las pantallas que `allauth` no trae hechas (esperar la verificación, pedir otro correo, corregir la dirección) | unidad 003 — "quién eres y cómo entras", y **solo eso** desde la unidad 023: `Usuario` ya no sabe de qué hogar es nadie ni tiene ningún dato personal colgando. Todo eso cuelga ahora de `hogares.Persona` |
| `hogares/` | `Hogar`, `Persona`, `SolicitudEntrada`, la base abstracta `ModeloDeHogar` (el aislamiento reutilizable) y la puerta única de acceso (`hogares/acceso.py`) | unidad 003 — "con quién compartes y quién ve qué"; toda unidad futura con datos del hogar (despensa, recetas, calendario) hereda de `ModeloDeHogar` en vez de reinventar el aislamiento. **Unidad 023**: nace `Persona` — quién vive en la casa — y de ella cuelgan el perfil, las pesadas, los entrenos, el plan del día y los cierres; `Usuario` se queda solo con "cómo entras". Toda cuenta estrena su persona con una señal (`hogares/signals.py`), y el id que viaja por las URLs (`<int:persona_id>`) es el de la PERSONA, no el de la cuenta |
...
| `progreso/` | la pantalla de "Progreso": la evolución de peso, grasa y cintura de la persona elegida, en SVG generado en el servidor | unidad 010 — sin modelos propios (reutiliza `perfiles.MedicionPeso`). `progreso/acceso.py` deja VER el progreso de cualquiera del hogar (R7), pero apuntar y borrar el peso siguen siendo solo de la propia persona, sin cambios, en `perfiles/` (R8): lectura y escritura tienen puertas distintas para el mismo dato. El cálculo (recorte por periodo, coordenadas del SVG) vive en `servicios/progreso.py`. Entrega PARCIAL a propósito: entrenos y cumplimiento (el resto del plano `ver-tu-progreso.md`) quedan pendientes, sin modelo de datos que los respalde todavía |
