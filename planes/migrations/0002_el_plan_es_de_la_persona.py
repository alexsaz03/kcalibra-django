"""
Unidad 023, paso 6 de 8 — el plan del día deja de colgar de la cuenta.

Mismo patrón de tres pasos que `perfiles.0004` (allí está escrito entero, incluido por qué
está prohibido un `AlterField` cambiando el `to=`). El campo `hogar` de `PlanDeDia` no se toca:
el plan sigue siendo DEL HOGAR (G-43), y ahora además dice de qué persona de ese hogar es.
"""
...
MOTIVO_IRREVERSIBLE = (
    ...
    "`hogares.Persona`— sus planes no tienen ningún `Usuario` al que volver, y una inversa "
...
def traducir_el_dueno(apps, schema_editor):
    ...
    PlanDeDia = apps.get_model("planes", "PlanDeDia")
    ...
    for usuario_id, persona_id in persona_de_la_cuenta.items():
        PlanDeDia.objects.filter(usuario_id=usuario_id).update(persona_id=persona_id)
    sin_dueno = PlanDeDia.objects.filter(persona__isnull=True).count()
    if sin_dueno:
        raise RuntimeError(
            f"{sin_dueno} plan(es) apuntan a un usuario que no tiene Persona. No se sigue: "
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RemoveConstraint(
            model_name="plandedia",
            name="un_plan_por_persona_y_dia",
        ),
        ...
        migrations.RemoveField(model_name="plandedia", name="usuario"),
